...
To create a local role for a project, it is recommended that you build it from
scratch rather than copying an existing role.
...
. Change the name (`exampleview`, for example).

. Change `kind` to `Role`.

. Remove `resourceVersion`, `selfLink`, `uid`, and `creationTimestamp`.

. Optionally, add `namespace: project-name`.
