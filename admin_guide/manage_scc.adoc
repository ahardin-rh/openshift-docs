...
the `securityContext.runAsUse`r field. The admission plug-in will look for the
`openshift.io/sa.scc.uid-range` annotation on the current namespace to populate
range fields, as it does not provide this range. In the end, a container will
have `runAsUser` equal to a semi-random value. The first value of the range is
hard to predict because every namespace has different ranges. See
